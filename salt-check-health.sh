...
log_report() {
    ...
    local report_line="| $test | \`$(escape_pipes "$command")\` | $(escape_pipes "$output") | $result |"
    
    # Add to REPORT_CONTENT
...
check_container() {
    ...
    if eval "$command 2>/dev/null | grep -q 'true'"; then
        ...
        if eval "$command; docker inspect -f '{{.State.Running}}' $container 2>/dev/null | grep -q 'true'"; then
...
check_connectivity() {
    ...
    if eval "$command 2>/dev/null | tee -a \"$LOG_FILE\" | grep -q \"True\""; then
        ...
        command="docker exec $MASTER salt-key -L"
        log "DEBUG" "Executing command: $command"
        ...
        command="docker exec $MASTER salt \"$node\" test.ping --out=txt"
        log "DEBUG" "Executing command: $command"
        if eval "$command 2>/dev/null | tee -a \"$LOG_FILE\" | grep -q \"True\""; then
            ...
            command="docker exec $MASTER salt --versions-report"
            log "DEBUG" "Executing command: $command"
            eval "$command | tee -a \"$LOG_FILE\""
            command="docker exec $node salt --versions-report"
            log "DEBUG" "Executing command: $command"
            eval "$command | tee -a \"$LOG_FILE\""
...
log INFO "Checking communication between syndics and master"

for syndic in "${SYNDICS[@]}"; do

    log INFO "Test if syndic can reach master through ping command."
...
log INFO "Check if syndic keys are accepted from master."
for syndic in "${SYNDICS[@]}"; do 
...
log INFO "Accept syndic keys from master."
for syndic in "${SYNDICS[@]}"; do 
...
log INFO "Ensure that the Salt services are running correctly on both the master and syndic."
for syndic in "${SYNDICS[@]}"; do 
   command="docker exec ${syndic} ps aux | grep salt-minion"
   log "DEBUG" "Executing command: $command"
...
for syndic in "${SYNDICS[@]}"; do 
   command="docker exec ${syndic} ps aux | grep salt-master"
   log "DEBUG" "Executing command: $command"
...
for syndic in "${SYNDICS[@]}"; do 
   command="docker exec ${syndic} ps aux | grep salt-syndic"
   log "DEBUG" "Executing command: $command"
...
log INFO "Check logs for any errors or warnings that may indicate what is wrong."
for syndic in "${SYNDICS[@]}"; do 
...
log INFO "Ensure that there are no firewall rules blocking communication between the syndic and master, especially on ports 4505 and 4506."
for syndic in "${SYNDICS[@]}"; do 
